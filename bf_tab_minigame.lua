    ...
    local function startQTE()
        ...
        qteConnection = RunService.Heartbeat:Connect(function()
            ...
            for _, obj in ipairs(pg:GetDescendants()) do
                ...
                pcall(function()
                    if isMobile then
                        -- iPad: InputBegan connections direkt feuern
                        local fired = false
                        pcall(function()
                            local conns = getconnections(obj.InputBegan)
                            if #conns > 0 then
                        ...
                        task.wait(0.05)
                        pcall(function()
                            local conns = getconnections(obj.InputEnded)
                            for _, c in ipairs(conns) do
                                c:Fire({
                                    UserInputType  = Enum.UserInputType.Touch,
            ...
            if value then
                ...
                speedConnection = RunService.Heartbeat:Connect(function()
                    ...
                    for _, obj in ipairs(pg:GetDescendants()) do
                        ...
                        pcall(function()
                            if isMobile then
                                -- iPad: getconnections InputBegan direkt feuern
                                local fired = false
                                pcall(function()
                                    local conns = getconnections(obj.InputBegan)
                                    if #conns > 0 then
