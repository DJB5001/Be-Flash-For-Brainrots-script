-- BF HUB — Be Flash For Brainrots Script
-- Author: DJB5001
...
function SaveSystem.load(name)
    ...
    if _G.__BF_Notify then _G.__BF_Notify("settings:applied") end
    return true
end
...
function SaveSystem.delete(name)
    ...
    if _G.__BF_Notify then _G.__BF_Notify("saves:changed") end
    return true
end
...
local subscribers = {}
_G.__BF_Subscribe = function(fn)  table.insert(subscribers, fn) end
_G.__BF_Notify    = function(evt) for _, fn in ipairs(subscribers) do pcall(fn, evt) end end
...
local function loadModule(name)
    ...
    if not src then
        warn("[BF HUB] Download failed: " .. name)
        return nil
    end
    local ok, chunk = pcall(loadstring, src)
    if not ok or not chunk then
        warn("[BF HUB] Compile failed: " .. name)
        return nil
    end
    local ok2, mod = pcall(chunk)
    if not ok2 then
        warn("[BF HUB] Execution failed: " .. name)
        return nil
    end
    return mod
end
...
-- BOOTSTRAP
-- ================================================================
print("[BF HUB] Starting " .. GAME_NAME .. " v" .. VERSION .. "...")

local Utils = loadModule("bf_utils.lua")
if not Utils then warn("[BF HUB] Utils failed to load") end

local Overlay = loadModule("bf_overlay.lua")
if Overlay then
    Overlay.showProgress(
        "Loading BF HUB v" .. VERSION .. "\nGame: " .. GAME_NAME,
        6
    )
end

local UIBase = loadModule("bf_ui_base.lua")
if not UIBase then
    error("[BF HUB] FATAL: UI base failed to load")
end

local Rayfield, Window = UIBase.createWindow()
if not Rayfield or not Window then
    error("[BF HUB] FATAL: Window could not be created")
end
...
local function onKeyVerified()
    ...
    for _, entry in ipairs(tabs) do
        ...
        if build then
            ...
            if ok then
                print("[BF HUB] " .. entry.label .. " tab loaded")
            else
                warn("[BF HUB] " .. entry.label .. " tab error: " .. tostring(err))
            end
        else
            warn("[BF HUB] Could not load: " .. entry.file)
        end
    end

    Rayfield:Notify({
        Title   = "BF HUB Ready!",
        Content = "Be Flash For Brainrots Script loaded!\nDiscord: discord.gg/MTXnFfHXW9",
        Duration = 6,
    })

    print("[BF HUB] All tabs loaded.")
end

-- ================================================================
-- KEY SYSTEM
-- ================================================================
local buildKey = loadModule("bf_tab_key.lua")
if buildKey then
    local ok, err = pcall(buildKey, Window, Rayfield, Utils, Config, onKeyVerified)
    if not ok then
        warn("[BF HUB] Key tab error: " .. tostring(err))
        onKeyVerified()
    end
else
    warn("[BF HUB] Key tab failed to load — skipping key check")
    onKeyVerified()
end

print("[BF HUB] Loader complete.")
